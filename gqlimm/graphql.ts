  ...
  selectionSet.selections.forEach(selection => {
    ...
    if (isField(selection)) {
      ...
      if (fieldResult !== undefined) {
        if (result.get('resultFieldKey') === undefined) {
          result = result.set(resultFieldKey, fieldResult);
        } else {
          result = merge(result[resultFieldKey], fieldResult);
        }
      }
    } else {
      ...
      if (execContext.fragmentMatcher(rootValue, typeCondition, contextValue)) {
        ...
        result = merge(result, fragmentResult);
      }
    }
  });
